...
var Map = React.createClass({
  render: function () {
    return <canvas id="grid" width="801px" height="601px"></canvas>;
  },
});
  ...
  render: function () {
    return (
      <div>
        <div>
          <h2>
            <b>XP</b>: <span id="xp">0</span>- <b>Level</b>: <span id="level">0</span>-{' '}
            <b>Health</b>: <span id="health">0</span>- <b>Weapon</b>: <span id="weapon">0</span>-{' '}
            <b>Damage</b>: <span id="damage">0</span>
          </h2>
        </div>
        <h3>
          Enemies left: <span id="enemies">0</span>
        </h3>
      </div>
    );
  },
...
function generateEnemies(amount) {
  for (var i = 0; i < amount; i++) {
    ...
    enemies.push(
      new Enemy(
        ENEMIES_HEALTH[Math.floor(Math.random() * ENEMIES_HEALTH.length)],
        coords,
        ENEMIES_DAMAGE[Math.floor(Math.random() * ENEMIES_DAMAGE.length)],
      ),
    );
